...
namespace Tanmo\OOS\Core;

/**
 * Class OosUtil
 *
 * Oss Util class for OosClient. The caller could use it for formating the result from OosClient.
 *
 * @package OOS
 */
class OosUtil
{
    const OOS_CONTENT = 'content';
    const OOS_LENGTH = 'length';
    const OOS_HEADERS = 'headers';
    const OOS_MAX_OBJECT_GROUP_VALUE = 1000;
    const OOS_MAX_PART_SIZE = 5368709120; // 5GB
    const OOS_MID_PART_SIZE = 10485760; // 10MB
    const OOS_MIN_PART_SIZE = 102400; // 100KB
     ...
     */
    public static function toQueryString($options = array(),$cmpFunction = NULL)
    {
        $temp = array();
        if(!isset($cmpFunction))
            $cmpFunction = 'strnatcasecmp';

        uksort($options, $cmpFunction);
    ...
    public static function validateOptions($options)
    {
        ...
        if ($options != NULL && !is_array($options)) {
            throw new OosException ($options . ':' . 'option must be array');
        }
    }
    ...
    {
        if (empty($content)) {
            throw new OosException("http body content is invalid");
        }
    }
...
BBB;
    ...
    {
        $content_md5 = "";
        if (($to_pos - $from_pos) > self::OOS_MAX_PART_SIZE) {
            return $content_md5;
        }
    ...
    public static function createDeleteObjectsXmlBody($objects, $quiet)
    {
        ...
        foreach ($objects as $object) {
            ...
            $object = OosUtil::sReplace($object);
    ...
    public static function decodeKey($key, $encoding)
    {
        ...
        if ($encoding == "url") {
            ...
            throw new OosException("Unrecognized encoding type: " . $encoding);
        }
    }
}
